...
class HandeyeCalibrationBackendOpenCV(object):
    ...
    def _msg_to_opencv(transform_msg):
        ...
        if isinstance(transform_msg, dict):
            ...
            tr = np.array((cmt['x'], cmt['y'], cmt['z']))   # get transfer matrix T
            cmq = transform_msg["orientation"]
            rot = tfs.quaternions.quat2mat(                 # get quaternion matrix R
            ...
            cmq = transform_msg.orientation
            rot = tfs.quaternions.quat2mat((cmq.w, cmq.x, cmq.y, cmq.z)) # Rotation matrix corresponding to input quaternion *q*
    ...
    def _get_opencv_samples(samples,eye_on_hand=True):
        ...
        for s in samples:
            ...
            (mcr, mct) = HandeyeCalibrationBackendOpenCV._msg_to_opencv( # mcr_quaternion matrix, mct_transfer matrix
                camera_marker_msg)
            # 眼在手外，手眼调换
            if not eye_on_hand:
        ...
        return (hand_base_rot, hand_base_tr), (marker_camera_rot, marker_camera_tr)     # return rotation matrixs and transfer matrix of each coordinates, all R and T
    ...
    def compute_calibration(self, samples, algorithm=None,eye_on_hand=True):
        ...
        result = tfs.affines.compose(np.squeeze(hand_camera_tr), hand_camera_rot, [1, 1, 1])# hand to camera
        ...
        for i in range(len(samples)):
            # base_link->end_link
            pose1 = tfs.affines.compose(np.squeeze(hand_world_tr[i]), hand_world_rot[i], [1, 1, 1]) # get a translation matrix, like [r, t; 000 1], 4x4, base to hand
            # marker->camera  or camera->marker
            pose2 = tfs.affines.compose(np.squeeze(marker_camera_tr[i]), marker_camera_rot[i], [1, 1, 1])       # camera to marker
            # base_link->end_link->marker   or base_link->end_link->camera 
            temp = np.dot(pose1,result)         # base to camera
            # base_link->end_link->marker->camera  or base_link->end_link->camera->marker
            temp = np.dot(temp,pose2)           # base to marker
            tr = temp[0:3,3:4].T[0]
