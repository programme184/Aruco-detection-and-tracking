...
real_moveit_pose=None

real_moveit_pose_sample=[]
...
def moveit_pose_callback(msg):
    global real_moveit_pose
    real_moveit_pose = msg.pose
    # print('callback test:', 1)
...
def calib_cmd_callback(msg):
    ...
    global img_num
    global real_moveit_pose
    global real_moveit_pose_sample
    ...
    if msg.position[0] ==1:
        ...
        pass
        with open(info_path+'arm_pose_info.txt', 'w') as file:
            # file.write(str(real_moveit_pose_sample))  
            for row in real_moveit_pose_sample:
...
def test_ros(online_pose_topic, camera_pose_topic, pose_moveit_topic):
    global real_online_pose, real_camera_pose, real_moveit_pose
    
    msg1 = rospy.wait_for_message(online_pose_topic, PoseStamped)
    real_online_pose = msg1.pose
    
    msg_cam = rospy.wait_for_message(camera_pose_topic, PoseStamped)
    real_camera_pose = msg_cam.pose
    
    msg_moveit = rospy.wait_for_message(pose_moveit_topic, PoseStamped)
    real_moveit_pose = msg_moveit.pose
...
if __name__ == '__main__':
    ...
    rospy.Subscriber(pose_moveit_topic, PoseStamped, moveit_pose_callback,queue_size=10)
    print("test subscriber:")
    # test_ros(online_pose_topic, camera_pose_topic, pose_moveit_topic)
    
    
    
    rospy.loginfo(real_moveit_pose)#, real_online_pose, real_camera_pose, real_moveit_pose
    rospy.loginfo(real_camera_pose)
    ...
    while not rospy.is_shutdown():
        ...
        # elif real_moveit_pose ==None:
        #     rospy.loginfo('等待机械臂moveit话题数据到位 ...')
        else:
            break
 
    print("test subscriber2:")
    print(real_online_pose)
    rospy.loginfo(real_moveit_pose)#, real_online_pose, real_camera_pose, real_moveit_pose
    rospy.loginfo(real_camera_pose)
    # while not rospy.is_shutdown():
    ...
    while not rospy.is_shutdown():
        ...
        if real_online_pose == None:
            rospy.loginfo('警告 等待机械臂位置和姿态话题到位 ...')
        elif real_camera_pose == None:
            rospy.loginfo('等待相机姿态话题数据到位 ...')
